...
Wir stellen dir dafür eine Reihe von strukturierten Aufgaben, die du nacheinander bearbeiten wirst. 

**Nachfrage-Prognose – Wende statistische Methoden an**  

Nachdem wir den Datensatz kennen gelernt haben, können wir im nächsten Schritt ein Modell entwickeln, mit dem wir die Nachfrage nach Leihfahrrädern vorhersagen können. Ziel ist es, deine Vorhersage letztendlich bei Kaggle einzureichen. Kaggle ist eine Plattform von Google, die regelmäßig "Competitions" startet, bei denen es darum geht etwas besonders gut vorherzusagen. Du bekommst einen Trainings-Datensatz mit dem du deine Modelle trainierst. Danach wendest du dein Modell auf einen seperaten Test-Datensatz an. Kaggle überprüft dann die Genauigkeit deiner Vorhersagen und rankt deine Lösung danach.  
...
Vielleicht wäre es besser, einen beliebigen 7-Tage Zeitraum auszuwählen (Tipp: Starte bei bei der ersten Beobachtung) und nur diesen zu plotten. Was kann man beobachten? So in etwa könnte dein Plot aussehen:
...
Herzlichen Glückwunsch – Du hast jetzt dank vieler Grafiken ein gutes Grundverständnis des Bike Sharing-Geschäfts. Damit hast du den ersten Teil des Projektes erfolgreich abgeschlossen! 
...
Du kommst nicht weiter? Oder willst dein Modell noch weiter verbessern, weißt aber nicht genau wie? Oder dir fällt gerade nicht ein, wie man etwas bestimmtes im Code umsetzt? Folgende Ansätze werden dich weiter bringen.  

## Cheat Sheets
Eine sehr nützliche Ressource sind die *Cheat Sheets* von RStudio. Wenn du einmal einen bestimmten Code-Baustein nachschauen willst, helfen diese grafisch schön aufbereiteten Spickzettel oft weiter:  

\begin{center} <https://www.rstudio.com/resources/cheatsheets/> \end{center}

## StackOverflow und CrossValidated
Ein großer Vorteil von R ist, dass die Programmiersprache open-source ist und eine sehr aktive Community existiert, die bei allen möglichen Fragen zur Verfügung steht. Falls du eine bestimmte Idee hast, aber nicht genau weißt wie du das umsetzen kannst wirst du sehr schnell durch googlen auf einen Lösungsweg gebracht. Meistens landest du dann auf einer Seite von *StackOverflow* für Fragen zu der Programmiersprache R

\begin{center}<https://stackoverflow.com/> .\end{center}  

Oder auf *CrossValidated*  

\begin{center}<https://stats.stackexchange.com/> ,\end{center}

der gleichen Seite für generelle Statistik-Fragestellungen. Dort beantworten Experten Fragen von Nutzern – und meistens wurde deine Frage schon einmal genau so oder leicht abgewandelt gestellt.

## Spreche deine Mentoren an!
Unsere Mentoren kennen sich gut in R aus und können dir wahrscheinlich oft einen guten Denkanstoß geben. Spreche sie gerne mit deinen Fragen an! Sei es über Slack oder im Rahmen der Coding Meetups.
